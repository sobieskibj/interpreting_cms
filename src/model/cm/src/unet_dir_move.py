...
class UNetModelDirMove(nn.Module):
    ...
    def register_names(self):
        """
        Assigns an id to each block.
        """

        for idx, block in enumerate(self.input_blocks):
            block.id = f'input_{idx}'

        self.middle_block.id = 'middle_0'

        for idx, block in enumerate(self.output_blocks):
            block.id = f'output_{idx}'
        

    def forward(self, x, timesteps, y = None, h_move = None):
        """
